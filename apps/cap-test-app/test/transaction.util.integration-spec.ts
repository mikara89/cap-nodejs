...
async function createAppWithMikro(opts: { clientUrl: string; entities: any[] }) {
    // dynamic require for driver
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const pgDriverModule = require('@mikro-orm/postgresql');
    const driver = pgDriverModule?.PostgreSqlDriver ?? pgDriverModule?.default ?? pgDriverModule;
...
describe('Integration: withTransactionAndPostCommit (MikroORM + Postgres)', () => {
    ...
    it('commits transactional save and runs afterCommitFn', async () => {
        ...
        expect((publisher.emit as jest.Mock).mock.calls.length).toBeGreaterThan(0);
    });
    ...
    it('rolls back transactional save and does not run afterCommitFn', async () => {
        ...
        expect((publisher.emit as jest.Mock).mock.calls.length).toBe(0);
    });
});
