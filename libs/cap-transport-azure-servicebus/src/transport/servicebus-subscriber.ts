...
export class ServiceBusSubscriber implements SubscriberPort {
  ...
  async initialize?(options?: {
    ...
    try {
      this.logger?.debug?.(
        `ServiceBusSubscriber.initialize() options`,
        options,
      );
  ...
  async consume(
    ...
    this.logger?.info?.(`subscribed to ${target.key}`);
  }
    ...
    if (!this.admin && this.createQueues) {
      ...
      if (storedCfg?.connectionString) {
        try {
          ...
          this.logger?.info?.(
            'ServiceBusSubscriber: admin client created lazily',
          );
        } catch (err) {
          this.logger?.warn?.(
  ...
  private async provisionQueueFallback(
    ...
    try {
      ...
      try {
        ...
        this.logger?.info?.(`Created queue ${target.resourceName}`);
  ...
  private async ensureTopic(
    ...
    try {
      ...
      try {
        ...
        this.logger?.info?.(`Created topic ${resourceName}`);
      } catch (err) {
        this.logger?.warn?.(
